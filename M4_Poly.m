...
%多项式
    %创建
        ...
        f1 = poly2str(p, 'x');  % 生成好看的"字符串" f1 = x^3 + 2 x^2 + 3 x + 4
        % 不被认可的运算式，上式不具备运算功能
        f2 = poly2sym(p);  % 生成可用的符号函数 f2 = x^3 + 2*x^2 + 3*x + 4
    %求值
        x = 4;
        y1 = polyval(p, x);  % 代入求值；若x1为矩阵，则对每个值单独求值，代入到方程
    %求根
        r = roots(p); % p同上，由系数求根，x^3 + 2*x^2 + 3*x + 4 = 0的根,结果为根植矩阵
...
%数据插值
    %一维插值
        ...
        y2m = interp1(X, Y, 2, 'spline');  % 用spline方法(三次样条)差值预估在x=2的y的值(还有其他选项详见help)
    %二维插值
...
%数据统计
    %矩阵最大最小值
        ...
        [y, k] = max(A);  % 求每一列最大值并以行向量形式返回，k为该值在该列的角标
        [y, k] = max(A, [], 2);% 前两个参数表示 A 矩阵与空矩阵合并
        % A是矩阵，'2'时返回y每一行最大元素构成的列向量，k元素所在列；'1'时与上述相同(求列中最大值)
    %均值和中值
        y = mean(X);  % 均值
        y = median(X);  % 中值
