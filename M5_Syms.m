...
%符号对象创建
    ...
    %syms函数
        syms x;  % 声明符号变量
        f = 7*x^2 + 2*x+9;  % 创建符号函数(符号函数就是用字母代替数字的函数，符号也就是变量)
    %符号运算
        % 加减乘除外
        % '转置 ； ==相等 ； ~=不等
...
%符号多项式函数运算
    %*符号表达形式与相互转化
        %多项式展开整理
            g = expand(f);  % 展开(把括号进行展开)
            h = collect(g);  % 整理(默认按x整理，进行降幂排列)
...
%符号微积分
    %函数的极限和级数运算
        ...
        %极限
            ...
            limit(f, 4);  % 默认变量->4
            limit(f);  % 默认变量->0
            limit(f, x, 4, 'right');  % 求f(x), x->4+(右极限), 'left' x->4-(左极限)
        %*基本级数运算
            %求和
...
%*符号方程求解
    %符号代数方程
        ...
        %多元方程组
            ...
            [Sxn, Syn,params,conds] = solve(eqn21, eqn22, x, y, 'ReturnCondition', true);
            % 加上参数ReturnCondition可返回通解及解的条c件
            % 其他参数(参数加上true生效)
