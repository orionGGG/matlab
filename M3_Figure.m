...
% 二维曲线绘制
    % 基本函数
        % plot(y)
            % y为向量
                plot(y1);  % 纵坐标为y1的值；横坐标自动为数组元素序号(角标+1)，此处为1~9
            % y为矩阵
...
% 线性图形格式设置
        ...
        % 坐标轴
            plot(x, y1);
            axis([-1*pi, 3*pi, -1.5, 1.5]);  % 规定横纵坐标范围(x轴最小值,x_max,y_min,y_max)
% 图形修饰
        % 标题标签
            title('a title');  % 图像标题
            xlabel('this is x');  % x轴标记，同理还有ylabel，zlabel
        %图例设置
            legend('hahaha', 'location', 'best');%(图例的文字，option，option对应的值)
...
%三维曲线曲面绘制
    ...
    %三维曲面
        %三维网格
            ...
            [X, Y] = meshgrid(x, y);  % 得到了xoy面网格点(生成曲面)
            Z = X.^2+Y.^2;
